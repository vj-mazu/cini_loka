...
const ProjectCard = ({ project, idx, onServiceSelect }: { project: any; idx: number; onServiceSelect: any }) => {
  ...
  return (
    <motion.div
      ...
      className={`${project.span} group relative overflow-hidden rounded-[2rem] md:rounded-[3rem] bg-surface border border-white/5 min-h-[320px] md:min-h-[550px] cursor-pointer shadow-2xl transition-all duration-700 hover:border-white/20 hover:shadow-accent/5`}
    >
      <motion.img
        src={project.image}
        alt={project.title}
      ...
      {/* Card Content */}
      <div className="absolute bottom-0 left-0 right-0 p-6 md:p-14 flex items-end justify-between">
        <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-700 ease-out">
          <span className="text-[10px] text-accent-light uppercase tracking-[0.5em] font-bold block mb-3 md:mb-4 opacity-70">
            {project.category}
          </span>
          <h3 className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-display italic text-white tracking-tighter leading-[1.1] mb-2 pb-2">
            {project.title}
          </h3>
        </div>
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-20 relative z-10">
        ...
        >
          <div className="max-w-3xl">
            ...
            <h2 className="text-4xl sm:text-6xl md:text-8xl lg:text-9xl font-display italic leading-[1.1] tracking-tighter pb-8">
              <span className="text-white/20">Our</span> <span className="gold-gradient">Space</span>
            </h2>
            <p className="text-text-secondary text-base md:text-xl max-w-xl mt-6 md:mt-10 leading-relaxed font-light">
        ...
        <motion.div
          ...
          className="mt-16 md:mt-32 p-6 sm:p-12 md:p-24 rounded-3xl md:rounded-[4rem] glass-dark border border-white/5 relative overflow-hidden flex flex-col items-center text-center"
        >
          ...
          <div className="relative z-10 max-w-3xl">
            ...
            <h3 className="text-3xl sm:text-5xl md:text-8xl font-display italic text-white mb-6 md:mb-10 tracking-tighter leading-[1.1]">
