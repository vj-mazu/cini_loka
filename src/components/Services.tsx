      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-20 relative z-10">
        ...
        >
          <div className="max-w-3xl">
            ...
            <h2 className="text-4xl sm:text-6xl md:text-8xl lg:text-9xl font-display italic leading-[1.1] tracking-tighter pb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {SERVICES.map((service, idx) => (
            <motion.div
              ...
              className="group relative glass rounded-[2rem] md:rounded-[3rem] p-6 md:p-14 transition-all duration-700 cursor-pointer hover:bg-white/[0.08] hover:border-white/20"
            >
              <div className="absolute inset-0 bg-gradient-to-br from-accent/10 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 rounded-[2rem]" />
              
              {/* Icon with float animation */}
              <div className="text-5xl mb-6 group-hover:scale-110 transition-transform duration-700 ease-out group-hover:animate-float">
                {service.icon}
              </div>

              {/* Title */}
              <h3 className={`text-2xl md:text-4xl font-display italic mb-4 tracking-tight leading-tight transition-colors duration-500 text-text-primary group-hover:text-accent-light`}>
                {service.title}
              </h3>
